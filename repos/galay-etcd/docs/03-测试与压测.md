...
`T1` 覆盖单 key 与 lease 生命周期，`T2` 覆盖 prefix range/delete，`T3` 覆盖 pipeline(txn success ops)，`T4` 覆盖异步客户端基本 KV 与 lease 操作，`T5` 覆盖异步 pipeline。
...
## 9. Sync 阻塞接口改造验证（2026-02-24）

改动：

- `EtcdClient` 从协程封装改为同步阻塞接口，返回 `std::expected<void, EtcdError>`
- `sync` 路径不再依赖 Awaitable/`co_await`
- 测试与 benchmark 调用方式统一改为阻塞调用
...
- 同步 HTTP 响应头解析修复：兼容最后一行 header 解析，避免误判缺失 `Content-Length`。
