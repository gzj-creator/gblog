...
Coroutine streamExample(RpcClient& client) {
    ...
    // 创建流连接
    RpcStreamConn stream(client.socket(), client.ringBuffer(), /*stream_id=*/1);
    auto& writer = stream.getWriter();
    auto& reader = stream.getReader();

    // 1. 发送初始化
    while (true) {
        auto r = co_await writer.sendInit("MyService", "streamMethod");
        if (!r || r.value()) break;
    }
    ...
    while (true) {
        ...
        while (true) {
            auto r = co_await reader.getMessage(recv_msg);
            if (!r || r.value()) break;
        }
...
ServiceEndpoint ep2{.host="10.0.0.2", .port=900n                    .service_name="EchoService", .instance_id="inst-2", .weight=200};
...
客户端调用支持通过 `.setTimeout(ms)` 设置超时：

```cpp
// 5 秒超时
while (true) {
    auto result = co_await client.call("Service", "method", "data").setTimeout(5000);
    if (!result) {
