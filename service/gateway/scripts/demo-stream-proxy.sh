...
find_static_bin() {
  ...
  local candidates=(
    "${REPO_ROOT}/build/gateway-debug/static-server"
    "${REPO_ROOT}/build/gateway/static-server"
    "${REPO_ROOT}/service/gateway/bin/static-server"
  )
...
[demo] set STATIC_BIN or build one first, for example:
  cmake -S service/gateway -B build/gateway-debug -DCMAKE_BUILD_TYPE=Debug -DCMAKE_PREFIX_PATH=/usr/local
  cmake --build build/gateway-debug --parallel
EOF
  exit 1
fi
...
RAW_CONF="${TMP_DIR}/gateway-raw.conf"
HTTP_CONF="${TMP_DIR}/gateway-http.conf"
RAW_LOG="${TMP_DIR}/gateway-raw.log"
HTTP_LOG="${TMP_DIR}/gateway-http.log"
