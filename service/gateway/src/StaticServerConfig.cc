...
struct IndexedRouteFields {
    ...
    std::optional<bool> preservePath;
};
...
std::optional<ProxyRouteConfig> parseRouteSpec(const std::string_view raw)
{
    ...
    if (fields.size() >= 5) {
        route.preservePath = parseBool(fields[4], false);
    }
...
void applyFileConfig(AppConfig& config, const std::vector<std::pair<std::string, std::string>>& entries)
{
    ...
    for (const auto& [key, value] : entries) {
        ...
        if (field == "preserve_prefix" || field == "keep_prefix" || field == "preserve_path") {
            current.preservePath = parseBool(value, false);
            continue;
        }
    }
    ...
    if (!indexedRouteFields.empty()) {
        ...
        for (const auto& id : ids) {
            ...
            if (fields.preservePath.has_value()) {
                route.preservePath = *fields.preservePath;
            }
            normalizeRoutePrefix(route.routePrefix);
