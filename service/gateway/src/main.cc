...
void registerPreservePrefixProxy(HttpRouter& router, const static_server::ProxyRouteConfig& route)
{
    auto passthrough = std::make_shared<HttpRouter>();
    passthrough->proxy("/", route.upstreamHost, route.upstreamPort, route.mode);

    HttpRouteHandler handler = [passthrough](HttpConn& conn, HttpRequest req) -> Coroutine {
        RouteMatch match = passthrough->findHandler(req.header().method(), req.header().uri());
        if (match.handler == nullptr) {
            co_return;
        }
        co_await (*match.handler)(conn, std::move(req)).wait();
        co_return;
    };

    const std::string wildcardPath = route.routePrefix == "/" ? "/**" : route.routePrefix + "/**";
    ...
    if (route.routePrefix != "/") {
        router.addHandler<HttpMethod::GET, HttpMethod::POST, HttpMethod::PUT,
                          HttpMethod::PATCH, HttpMethod::DELETE, HttpMethod::HEAD,
                          HttpMethod::OPTIONS>(route.routePrefix, handler);
    }
}
        ...
        if (appConfig.proxyRoutes.empty()) {
            ...
            for (const auto& route : appConfig.proxyRoutes) {
                if (route.preservePath) {
                    registerPreservePrefixProxy(router, route);
                } else {
                    router.proxy(route.routePrefix, route.upstreamHost, route.upstreamPort, route.mode);
                }

                HTTP_LOG_INFO("[proxy] [config] [enabled] [prefix={}] [upstream={}:{}] [mode={}] [preserve_path={}]",
                              route.routePrefix,
                              route.upstreamHost,
                              route.upstreamPort,
                              route.mode == ProxyMode::Raw ? "raw" : "http",
                              route.preservePath ? "true" : "false");
            }
        }
    } else {
